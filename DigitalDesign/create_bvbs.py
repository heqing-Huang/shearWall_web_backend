...
from DoubleWallDesign.models import RebarforBVBS, RebarBVBS
...
class BVBS():
    def __init__(self, project_number: str = '1', schedule_number: str = '1', revision: str = '', bar_mark: str = '1',
                 bar_length: str = '1000', bar_quantity: str = '1', bar_diameter: str = '12',
                 steel_grade: str = 'HPB300',
                 mandrel_diameter: str = '', mesh_type: str = '', mesh_width: str = '', designer: str = 'CR',
                 layer: str = '', delta: str = '', group: str = ''):
        '''
        创建bvbs数据
        ...
        self.bar_weight = 'e' + str(round(7.85 * math.pi * float(bar_diameter) ** 2 / 4 * float(bar_length) / 1e6, 2))
    ...
    def create_BF2D(self, Geometry_block=None, Spacer_block=None, Bar_block=None, Coupler_block=None):
        self.bvbs_string += 'BF2D@'
        # 增加H字节
        self.bvbs_string += 'H'
        BF2D_H = [self.project_number, self.schedule_number, self.revision, self.bar_mark, self.bar_length,
                  self.bar_quantity, self.bar_weight, self.bar_diameter, self.steel_grade, self.mandrel_diameter,
                  self.designer, self.delta, self.group]
        ...
        if Geometry_block:
            ...
            for geo in Geometry_block:
                geo: RebarGeoBVBS
                self.bvbs_string += 'l' + str(geo.length) + '@' + 'w' + str(geo.angle) + '@'
        elif Spacer_block:
            pass
        elif Bar_block:
            pass
...
def create_bvbs(ifc_doc: IfcDocument, rebar_for_BVBS: RebarforBVBS, shear_wall_data: ShearWallData):
    ...
    ascii_strings = ''
    for attr, type in rebar_for_BVBS.__dict__.items():
        for value in type:
            value: RebarBVBS
            bvbs = BVBS(project_number=value.project_ID, schedule_number=value.shear_wall_ID, bar_mark=str(value.mark),
                        bar_length=str(value.rebar_length), bar_quantity=str(value.rebar_quantity),
                        bar_diameter=str(value.rebar_diameter), steel_grade=value.rebar_grade,
                        mandrel_diameter=str(value.mandrel_diameter))
            ascii_strings += bvbs.create_BF2D(Geometry_block=value.geometric)

    save_string_to_file(ascii_strings)


# Saves the resulting string to the directory runnning the script.
def save_string_to_file(ascii_strings):
    with open("../bvbs_code.abs", "w") as f:
        try:
