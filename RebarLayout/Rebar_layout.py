...
def find_next_point(bar_dia, fcl_model, current_point_find, offset, action_1, action_2, vecN):
    if offset == 0:
        L1 = 0
        L2 = 0
...
def APF(st_p, object_p, fcl_model, bar_dia, offset, normal_vector, qa=1):
    ...
    '''
    step_length = 3
    init_dir = (object_p - st_p) / np.linalg.norm((object_p - st_p))
    label = np.argmax(np.abs(object_p - st_p))
    ...
    for kstep in range(5000):
        ...
        att_azimuth, att_elevation = compute_aer(current_point, object_p_tem, att_r)
        Fatx, Faty, Fatz = compute_Attract(qa, att_azimuth, att_elevation, att_r)
        Fa = np.array([Fatx, Faty, Fatz])
        vec1 = step_length * Fa / np.sqrt(np.sum(Fa * Fa))
        ...
        if check_ob == 1:
            ...
            next_point = find_next_point(bar_dia, fcl_model, current_point_find, offset, action_1, action_2, vecN)
...
def APF_rebar(fcl_model, bar, bar_dia, offset, normal_vector, qa=1):
    ...
    rebar_fcls = fcl_model.new_rebar_fcl(check_rebar, bar_dia)
...
def long_rebar_data_convert(rebar_list, rebar_dia):
    ...
    for i in range(len(rebar_list)):
        ...
        for j in range(len(rebar_list[i])):
            rebar_set.append([rebar_list[i][j].x, rebar_list[i][j].y,
                              rebar_list[i][j].z])
        rebar_sets.append(rebar_set)
...
def mid_rebar_convert(mid_rebar_computation):
    '''
    整理分布钢筋数据
    :param mid_rebar_computation:
    :return:
    '''
    mid_rebar_set = []
    for i in range(int(len(mid_rebar_computation) / 2)):
        mid_rebar_set = mid_rebar_set + [
            [[mid_rebar_computation[2 * i][1][0], mid_rebar_computation[2 * i][1][1],
              mid_rebar_computation[2 * i][1][2]],
             [mid_rebar_computation[2 * i][2][0], mid_rebar_computation[2 * i][2][1],
              mid_rebar_computation[2 * i][2][2]]],
            [[mid_rebar_computation[2 * i + 1][1][0], mid_rebar_computation[2 * i + 1][1][1],
              mid_rebar_computation[2 * i + 1][1][2]],
             [mid_rebar_computation[2 * i + 1][2][0], mid_rebar_computation[2 * i + 1][2][1],
...
def dis_rebar_data_convert(rebar_list, rebar_dia):
    ...
    for i in range(int(len(rebar_list) / 2)):
        rebar_sets = [[[rebar_list[2 * i][1].x, rebar_list[2 * i][1].y, rebar_list[2 * i][1].z],
                       [rebar_list[2 * i][2].x, rebar_list[2 * i][2].y, rebar_list[2 * i][2].z]],
                      [[rebar_list[2 * i + 1][1].x, rebar_list[2 * i + 1][1].y, rebar_list[2 * i + 1][1].z],
                       [rebar_list[2 * i + 1][2].x, rebar_list[2 * i + 1][2].y, rebar_list[2 * i + 1][2].z]]]
        rebar_computation = rebar_computation + rebar_sets
    rebar_computation = np.array(rebar_computation)
    return rebar_computation, rebar_dias
...
class RebarSet():
    def __init__(self, fcl_model, rebar_list, rebar_dias, normal_vector=np.array([1, 0, 0])):
    ...
    def long_layout(self):
        ...
        offset_label = np.argmax(self.normal_vector)
        rebar_cell = [[] for _ in range(len(self.rebar_list))]
        for i in range(len(self.rebar_list)):
            rebar = copy.deepcopy(self.rebar_list[i])
            rebar_cell[i].append(rebar[0].tolist())
            ...
            for j in range(len(rebar) - 1):
                ...
                rebar_points = APF(st_p=start_point, object_p=object_point, fcl_model=self.fcl_model,
                                   bar_dia=self.rebar_dias[i], offset=offset, normal_vector=self.normal_vector)
                points_offset = PointOffset(rebar_points)
                points_offset.featurepoint()
                rebar_points_offset = points_offset.long_picknewpoint()
                ...
                if offset == 0:
                    if rebar_points_offset[0][offset_label] - origin > 0:
                        ...
                        offset = 0
                rebar[:, [offset_label]] = rebar_points_offset[-1][offset_label]
            rebar_cell[i] = point_offset_rebar(rebar_cell[i], origin, offset_label)
    ...
    def dis_layout(self):
        ...
        for i in range(int(len(self.rebar_list) / 2)):
            ...
            rebar_points_bottom = APF(st_p=start_point_bottom, object_p=object_point_bottom, fcl_model=self.fcl_model,
                                      bar_dia=self.rebar_dias[2 * i], offset=offset, normal_vector=self.normal_vector)
            ...
            rebar_points_top = APF(st_p=start_point_top, object_p=object_point_top, fcl_model=self.fcl_model,
                                   bar_dia=self.rebar_dias[2 * i + 1], offset=offset, normal_vector=self.normal_vector)
            ...
            self.fcl_model.add_rebar(rebar=rebar_bottom_output, dia=self.rebar_dias[2 * i])
            self.rebar_output[2 * i] = rebar_bottom_output
            self.fcl_model.add_rebar(rebar=rebar_top_out, dia=self.rebar_dias[2 * i + 1])
    ...
    def stirrup_layout(self):
        ...
        for i in range(len(self.rebar_list)):
            ...
            origin = copy.deepcopy(rebar[0][offset_label])  # 偏移 初始点
            offset = 0  # 偏移方向
            rebar_set = APF_rebar(fcl_model=self.fcl_model, bar=rebar,
                                  bar_dia=self.rebar_dias[i], offset=offset, normal_vector=self.normal_vector)
...
def mid_rebar_for_BIM(rebar_bottom, rebar_top):
    rebar_bottom_point = [[rebar_bottom[0][0], rebar_top[0][1], rebar_top[0][2]],
                          rebar_bottom[0].tolist(),
                          rebar_bottom[1].tolist(),
                          [rebar_bottom[1][0], rebar_top[0][1], rebar_top[0][2]]]

    rebar_top_point = [[rebar_top[0][0], rebar_bottom[0][1], rebar_bottom[0][2]],
                       rebar_top[0].tolist(),
                       rebar_top[1].tolist(),
...
def rebar_layout(detailed_design: DetailedDesign, detailed_design_result: DetailedDesignResult):
    '''
    钢筋排布函数
    :param detailing_design:  深化设计参数
    :return: 用于BIM建模的钢筋数据
    '''
    ...
    stair_obstacle = ShearWallObstacle(detailed_design=detailed_design, detailed_design_result=detailed_design_result)

    # 保护层障碍物
    cover_objs = stair_obstacle.get_cover()
    ...
    # 钢筋数据
    rebar_data = RebarData(detailed_design=detailed_design, detailed_design_result=detailed_design_result)
    # 孔洞加强钢筋
    hole_rebar_BVBS, hole_rebar_objs, hole_rebar_BIM = rebar_data.get_hole_rebar()
    rebar_for_BVBS.hole_rebar = hole_rebar_BVBS
    rebar_for_BIM.hole_rebar = hole_rebar_BIM
    fcl_model.add_obj(hole_rebar_objs)

    # 吊装加强钢筋纵筋
    lifting_longitudinal_rebar_BVBS, lifting_longitudinal_rebar_objs, lifting_longitudinal_rebar_BIM = rebar_data.get_lifting_longitudinal_rebar()
    ...
    # 吊装加强钢筋点筋
    lifting_point_rebar_BVBS, lifting_point_rebar_objs, lifting_point_rebar_BIM = rebar_data.get_lifting_point_rebar()
    ...
    start = time.time()
    # 下部加强纵筋边筋
    bottom_edge_rein_rebar_BVBS, bottom_edge_reinforce_rebar = rebar_data.get_bottom_edge_reinforce_rebar()
    rebar_for_BVBS.bottom_edge_rein_rebar = bottom_edge_rein_rebar_BVBS
    bottom_edge_rein_rebar_computation = np.array(bottom_edge_reinforce_rebar)
    bottom_edge_rein_rebar_dias = np.ones(bottom_edge_rein_rebar_BVBS.rebar_quantity) \
                                  * bottom_edge_rein_rebar_BVBS.rebar_diameter
    bottom_edge_rein_rebar_layout = RebarSet(fcl_model, bottom_edge_rein_rebar_computation, bottom_edge_rein_rebar_dias)
    bottom_edge_rein_rebar_layout.long_layout()
    fcl_model = bottom_edge_rein_rebar_layout.fcl_model
    bottom_edge_rein_rebar_BIM = []
    for rebar_points in bottom_edge_rein_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(bottom_edge_rein_rebar_BVBS.rebar_diameter / 2),
    ...
    # 上部加强纵筋边筋
    top_edge_rein_rebar_BVBS, top_edge_reinforce_rebar = rebar_data.get_top_edge_reinforce_rebar()
    ...
    top_edge_rein_rebar_dias = np.ones(top_edge_rein_rebar_BVBS.rebar_quantity) * \
                               top_edge_rein_rebar_BVBS.rebar_diameter
    top_edge_rein_rebar_layout = RebarSet(fcl_model, top_edge_rein_rebar_computation, top_edge_rein_rebar_dias)
    ...
    for rebar_points in top_edge_rein_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(top_edge_rein_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list))
        top_edge_rein_rebar_BIM.append(rebar)
    rebar_for_BIM.top_edge_rein_rebar = top_edge_rein_rebar_BIM

    # 底部钢筋
    bottom_rebar_BVBS, bottom_rebar = rebar_data.get_bottom_rebar()
    rebar_for_BVBS.bottom_rebar = bottom_rebar_BVBS
    bottom_rebar_computation = np.array(bottom_rebar)
    bottom_rebar_dias = np.ones(bottom_rebar_BVBS.rebar_quantity) * bottom_rebar_BVBS.rebar_diameter
    bottom_rebar_layout = RebarSet(fcl_model, bottom_rebar_computation, bottom_rebar_dias)
    ...
    for rebar_points in bottom_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(bottom_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list))
        bottom_rebar_BIM.append(rebar)
    rebar_for_BIM.bottom_rebar = bottom_rebar_BIM

    # 顶部钢筋
    top_rebar_BVBS, top_rebar = rebar_data.get_top_rebar()
    rebar_for_BVBS.top_rebar = top_rebar_BVBS
    top_rebar_computation = np.array(top_rebar)
    top_rebar_dias = np.ones(top_rebar_BVBS.rebar_quantity) * top_rebar_BVBS.rebar_diameter
    ...
    for rebar_points in top_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(top_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list))
        top_rebar_BIM.append(rebar)
    rebar_for_BIM.top_rebar = top_rebar_BIM

    # 下部加强箍筋 
    bottom_edge_stirrup_rebar_BVBS, bottom_edge_stirrup_rebar = rebar_data.get_bottom_edge_stirrup_rebar()
    rebar_for_BVBS.bottom_edge_stirrup_rebar = bottom_edge_stirrup_rebar_BVBS
    bottom_edge_stirrup_rebar_computation = np.array(bottom_edge_stirrup_rebar)
    bottom_edge_stirrup_rebar_dias = np.ones(
        bottom_edge_stirrup_rebar_BVBS.rebar_quantity) * bottom_edge_stirrup_rebar_BVBS.rebar_diameter
    bottom_edge_stirrup_rebar_layout = RebarSet(fcl_model, bottom_edge_stirrup_rebar_computation,
    ...
    for rebar_points in bottom_edge_stirrup_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(bottom_edge_stirrup_rebar_BVBS.rebar_diameter / 2),
                      poly=IndexedPolyCurve(rebar_point_list))
        bottom_edge_stirrup_rebar_BIM.append(rebar)
    rebar_for_BIM.bottom_edge_stirrup_rebar = bottom_edge_stirrup_rebar_BIM

    # 上部加强箍筋
    top_edge_stirrup_rebar_BVBS, top_edge_stirrup_rebar = rebar_data.get_top_edge_stirrup_rebar()
    ...
    top_edge_stirrup_rebar_dias = np.ones(
        top_edge_stirrup_rebar_BVBS.rebar_quantity) * top_edge_stirrup_rebar_BVBS.rebar_diameter
    top_edge_stirrup_rebar_layout = RebarSet(fcl_model, top_edge_stirrup_rebar_computation, top_edge_stirrup_rebar_dias)
    ...
    for rebar_points in top_edge_stirrup_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(top_edge_stirrup_rebar_BVBS.rebar_diameter / 2),
                      poly=IndexedPolyCurve(rebar_point_list))
        top_edge_stirrup_rebar_BIM.append(rebar)
    rebar_for_BIM.top_edge_stirrup_rebar = top_edge_stirrup_rebar_BIM

    # 下部加强筋（分布）
    bottom_rein_rebar_BVBS, bottom_rein_rebar = rebar_data.get_bottom_rein_rebar()
    rebar_for_BVBS.bottom_rein_rebar = bottom_rein_rebar_BVBS
    bottom_rein_rebar_computation = np.array(bottom_rein_rebar)
    bottom_rein_rebar_dias = np.ones(bottom_rein_rebar_BVBS.rebar_quantity) * bottom_rein_rebar_BVBS.rebar_diameter
    bottom_rein_rebar_normal_vector = np.array([0, 1, 0])
    bottom_rein_rebar_layout = RebarSet(fcl_model, bottom_rein_rebar_computation, bottom_rein_rebar_dias,
                                        normal_vector=bottom_rein_rebar_normal_vector)
    bottom_rein_rebar_layout.dis_layout()
    ...
    for rebar_points in bottom_rein_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(bottom_rein_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list))
        bottom_rein_rebar_BIM.append(rebar)
    rebar_for_BIM.bottom_rein_rebar = bottom_rein_rebar_BIM

    # 上部加强筋 （分布）
    top_rein_rebar_BVBS, top_rein_rebar = rebar_data.get_top_rein_rebar()
    rebar_for_BVBS.top_rein_rebar = top_rein_rebar_BVBS
    top_rein_rebar_computation = np.array(top_rein_rebar)
    top_rein_rebar_dias = np.ones(top_rein_rebar_BVBS.rebar_quantity) * top_rein_rebar_BVBS.rebar_diameter
    top_rein_rebar_normal_vector = np.array([0, 1, 0])
    top_rein_rebar_layout = RebarSet(fcl_model, top_rein_rebar_computation, top_rein_rebar_dias,
                                     normal_vector=top_rein_rebar_normal_vector)
    top_rein_rebar_layout.dis_layout()
    fcl_model = top_rein_rebar_layout.fcl_model
    top_rein_rebar_BIM = []
    for rebar_points in top_rein_rebar_layout.rebar_output:
        rebar_point_list = rebar_points.tolist()
        rebar = Rebar(radius=int(top_rein_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list))
        top_rein_rebar_BIM.append(rebar)
    rebar_for_BIM.top_rein_rebar = top_rein_rebar_BIM

    # 分布筋
    mid_rebar_BVBS, mid_rebar = rebar_data.get_mid_rebar()
    rebar_for_BVBS.mid_rebar = mid_rebar_BVBS
    mid_rebar_computation = np.array(mid_rebar)
    mid_rebar_dias = np.ones(mid_rebar_BVBS.rebar_quantity) * mid_rebar_BVBS.rebar_diameter
    vector1 = mid_rebar_computation[0][1] - mid_rebar_computation[0][0]
    vector2 = mid_rebar_computation[0][2] - mid_rebar_computation[0][0]
    mid_rebar_normal_vector = np.cross(vector1, vector2) / np.linalg.norm(np.cross(vector1, vector2))
    mid_rebar_computation = mid_rebar_convert(mid_rebar_computation)
    mid_rebar_layout = RebarSet(fcl_model, mid_rebar_computation, mid_rebar_dias, normal_vector=mid_rebar_normal_vector)
    ...
    for i in range(int(len(mid_rebar_layout.rebar_output) / 2)):
        ...
        rebar_point_list_bottom, rebar_point_list_top = mid_rebar_for_BIM(original_rebar_bottom, original_rebar_top)
        rebar = Rebar(radius=int(mid_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list_bottom))
        rebar_for_BIM.mid_rebar.append(rebar)
        rebar = Rebar(radius=int(mid_rebar_BVBS.rebar_diameter / 2), poly=IndexedPolyCurve(rebar_point_list_top))
        rebar_for_BIM.mid_rebar.append(rebar)
    fcl_model = mid_rebar_layout.fcl_model
    stop = time.time()

    print(stop - start)
    return rebar_for_BIM, rebar_for_BVBS
